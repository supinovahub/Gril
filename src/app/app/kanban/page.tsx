...
export default async function KanbanPage() {
  ...
  const [{ data: stages }, { data: opportunities }] = await Promise.all([
    ...
    supabase.from("opportunities").select("id,status,source,last_activity_at,pipeline_stage_id,assigned_membership_id,contacts!inner(name,contact_phones(e164,is_primary,status))").eq("org_id", viewer.organization!.id).order("last_activity_at", { ascending: false }),
  ]);
                ...
                {cards.map((card) => {
                  ...
                  return (
                    <Link className={styles.kanbanCard} href={`/app/leads/${card.id}`} key={card.id}>
                      ...
                      <small>{card.source} · {card.assigned_membership_id ? "atribuído" : "sem responsável"}</small>

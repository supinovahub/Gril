import { Building2, CirclePause, Clock3, Landmark, RadioTower, ShieldCheck } from "lucide-react";
...
export default async function OrganizationSettingsPage({ searchParams }: { searchParams: Promise<{ erro?: string;sucesso?: string }> }) {
  ...
  return <div className={styles.page}>
    ...
    <section className={styles.settingsSummary} aria-label="Resumo da organização">
      <span><Landmark aria-hidden="true" size={17}/><small>Identidade</small><strong>{institutional.company_name && institutional.creci ? "Completa" : "Revisar"}</strong></span>
      <span><Building2 aria-hidden="true" size={17}/><small>Operação padrão</small><strong>{operation?.name ?? "Pendente"}</strong></span>
      <span><Clock3 aria-hidden="true" size={17}/><small>Atendimento</small><strong>{operationSettings ? `${String(operationSettings.inbound_window_start).slice(0,5)} até ${String(operationSettings.inbound_window_end).slice(0,5)}` : "Pendente"}</strong></span>
      <span><RadioTower aria-hidden="true" size={17}/><small>Automações</small><strong>{pauses?.length ? "Pausadas" : "Ativas"}</strong></span>
    </section>
    <nav aria-label="Áreas de Organização e operação" className={styles.settingsTabs}>
      <a href="#identidade">Identidade</a><a href="#operacao">Operação</a><a href="#seguranca">Segurança</a>
    </nav>
    ...
    <div className={styles.layout}><main className={styles.stack}>
      <form action={updateInstitutionalSettingsAction} className={styles.formCard} id="identidade"><h2>Identidade institucional</h2>
      ...
      {operation && operationSettings ? <form action={updateOperationSettingsAction} className={styles.formCard} id="operacao"><h2>Regras da operação</h2><input name="operationId" type="hidden" value={operation.id} />
    ...
    </main><aside className={styles.stack} id="seguranca"><section className={styles.panel}><h2>Gate de produção</h2><p className={styles.definition}><ShieldCheck size={15} /> O banco exige identidade, persona, regras, qualificação, conhecimento válido, modelo principal e fallback, canal saudável e regressão real aprovada.</p></section>

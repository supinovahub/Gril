...
| Privacidade e retenção | Opt-out, supressão, solicitações, hold e purge físico de Storage | Implementado; fases 7 e 10; purge real pendente |
...
| Especificação §§ 36–41: arquitetura, fluxos, aceite e ordem | Next.js + Supabase + worker + Vault | 41 migrações; 197 verificações | implementada e rastreada |
...
| Eventos § 14: testes obrigatórios | pgTAP + Vitest | `supabase/tests` e `src/**/*.test.ts` | 197/197 aprovadas |
| Mapa de Telas §§ 1–14: rotas, navegação e superfícies | 31 rotas/páginas geradas no build | build Next.js | existência/tipagem comprovadas; revisão visual pendente |
...
| Modelo de Dados §§ 1–11: schemas e entidades | 139 tabelas públicas, schemas `private` e `audit` | fases 1–23 | migrações remotas aplicadas |
| Modelo de Dados §§ 12–17: acesso, RLS, constraints, privacidade e validação | policies, funções security-definer e Vault | hardening final; testes 1–11 | 139/139 RLS; grants mínimos comprovados |
| Backlog §§ 1–4: DoD, épicos e dependências | decisões de fase 1–23 | 41 migrações | concluída a parte de código |
| Backlog § 5: estratégia de testes | Vitest, pgTAP e mocks de provedores | 70 + 127 | automatizada sem UI |
...
| README: decisões estruturantes e portões | este documento, readiness e evidências | lint, build, 197 testes e advisor | gate técnico automatizado concluído |
...
- **Segurança:** 139 tabelas públicas estão com RLS, zero grants de tabela para `anon` e zero privilégios excessivos de `authenticated`; segredos ficam no Vault; controles determinísticos precedem a IA; pausa global e auditoria existem.

...
|---|---|---|
| Organização, usuários, papéis e acesso | Auth, memberships, operações, convites, permissões e RLS | Implementado; `phase_01_rls.sql` 12/12 |
...
| Especificação §§ 36–41: arquitetura, fluxos, aceite e ordem | Next.js + Supabase + worker + Vault | 46 migrações; 200 verificações previamente aprovadas | implementada e rastreada |
...
| Eventos § 14: testes obrigatórios | pgTAP + Vitest | `supabase/tests` e `src/**/*.test.ts` | 127 pgTAP prévios + 73 Vitest atuais; novo fechamento pgTAP adicionado |
| Mapa de Telas §§ 1–14: rotas, navegação e superfícies | 34 rotas geradas no build | build Next.js | existência/tipagem comprovadas; revisão visual pendente |
...
| Modelo de Dados §§ 1–11: schemas e entidades | 144 tabelas públicas, schemas `private` e `audit` | fases 1–23 + fechamento | migrações remotas aplicadas |
| Modelo de Dados §§ 12–17: acesso, RLS, constraints, privacidade e validação | policies, funções com invocação restrita e Vault | hardening final; testes 1–12 | 144/144 RLS; grants mínimos comprovados |
| Backlog §§ 1–4: DoD, épicos e dependências | decisões de fase 1–23 | 46 migrações | núcleo de código concluído; P1/P2 explícitos no guia |
| Backlog § 5: estratégia de testes | Vitest, pgTAP e mocks de provedores | 73 + 127 previamente aprovados | automatizada sem UI |
...
| README: decisões estruturantes e portões | este documento, readiness e evidências | lint, build, 200 testes prévios, fechamento estrutural e advisors | gate técnico automatizado concluído |
...
- **Segurança:** 144 tabelas públicas estão com RLS, zero grants de tabela para `anon`; segredos ficam no Vault; controles determinísticos precedem a IA; pausa global e auditoria existem.

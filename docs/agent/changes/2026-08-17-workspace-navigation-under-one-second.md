...
- Depois: Inbox e Visão geral usam um bootstrap autenticado por rota e uma única viagem à Data API. A autorização continua vinculada a `auth.uid()`, organização, operação, papel, escopo, grant de conversa e suporte contratual, mas é materializada uma vez antes das agregações. Em cinco repetições aquecidas com o proprietário real, Inbox ficou entre 146 e 183 ms e Visão geral entre 128 e 169 ms; com corretor, ficaram entre 101 e 241 ms e 115 e 189 ms. Uma sessão sintética recém-criada, medida imediatamente após o OTP, levou 1.119 ms na primeira Inbox e 184–217 ms nas seguintes; o cenário solicitado de troca dentro da sessão ficou abaixo do limite e os contratos foram aquecidos antes do deploy de preview.

## Escopo executado

- Arquivos: `src/app/app/page.tsx`, `src/app/app/inbox/page.tsx`, `src/lib/auth/session.ts`, `src/lib/database.types.ts`, `supabase/tests/phase_40_workspace_navigation_latency.sql`, este registro, `docs/agent/CURRENT_STATE.md` e `docs/operations/GUIA_COMPLETO_DE_HOMOLOGACAO.md`.
...
- Mudanças externas em Supabase, Vercel, GitHub ou fornecedores: as quatro migrations foram aplicadas de forma serializada no projeto Supabase remoto `frslhzwhaooqtivkzdez`. Foram abertas sessões temporárias sem persistência local para medir proprietário e corretor pela Data API; nenhum dado de negócio foi alterado. O PR #48 será atualizado e uma nova preview automática será gerada; produção permanece inalterada.
...
- Deploy necessário: nova preview automática do PR #48; nenhum deploy de produção foi autorizado.
...
- Confirmar na nova preview, pelos runtime logs do artefato Vercel, que `inbox.workspace` e `dashboard.workspace` permanecem abaixo de um segundo no percurso real.

...
begin
  ...
  if jsonb_typeof(v_variants) <> 'array'
     ...
     or (select count(distinct trim(item->>'id')) from jsonb_array_elements(v_variants) item) <> jsonb_array_length(v_variants)
     or (select count(distinct trim(item->>'template')) from jsonb_array_elements(v_variants) item) <> jsonb_array_length(v_variants)
     or exists (
       select 1
...
-- The wave is the unit that matters for message diversity. The contact order
-- is the same order used to release the wave, so eligible contacts rotate
-- through the campaign pack only when they are actually queued.
...
begin
  ...
  select coalesce(max(wave_number), 0) + 1 into v_wave_no
  ...
  select greatest(
    now(), coalesce(max(run_at) + interval '1 minute', now())
  ) into v_slot
  ...
  loop
    ...
    if v_phone is null
      ...
      v_variant := v_contact.variant;
      if jsonb_array_length(v_campaign.opening_variants) > 0 then
        select item->>'id' into v_variant
        from jsonb_array_elements(v_campaign.opening_variants) with ordinality as entries(item, ordinal)
        where ordinal = mod(v_released, jsonb_array_length(v_campaign.opening_variants)) + 1
        limit 1;
      end if;
      ...
      update public.campaign_contacts
      set status = 'queued', wave_id = v_wave, variant = v_variant,
          next_send_at = v_slot, last_revalidated_at = now(), updated_at = now()
      where id = v_contact.id;
  ...
  insert into audit.events(
    ...
    jsonb_build_object(
      'released', v_released, 'suppressed', v_suppressed, 'wave', v_wave_no,
      'review_required', 0, 'review_gate', 'disabled'
    )
  );
  return new;
end;

...
-- The product contract uses a one-hour minimum lead time. Keep the lower
-- bound inside the database function so callers cannot bypass it by passing
-- an earlier p_from value.
...
begin
  ...
  with candidate_slots as (
    ...
    where slot >= greatest(p_from,now()+interval '1 hour')
  ), available_slots as (
...
comment on function public.get_pedro_available_call_slots(uuid,timestamptz,timestamptz,integer)
  is 'Returns only call slots with at least one hour of lead time.';
